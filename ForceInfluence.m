...
semilogy(E,StressdiffForceE(1,:),'-', E,StressdiffForceE(2,:),'--', E,StressdiffForceE(3,:),':', E,StressdiffForceE(4,:),'-.','LineWidth',2);
    title({['Standard deviation'] ['of the stress distribution as a function of E'] ['t =' num2str(t) 'mm and c =' num2str(c) ', k_0 =' num2str(k) 'kPa/mm']});
    xlabel('E [MPa]');
    ylabel('\sigma [kPa]');
    grid on
    lgd = legend(string(Load/1000),'location','northeast');
    title(lgd,'Axle load [kN]')
...
plot(E,MaxSpringForceE(1,:),'-', E,MaxSpringForceE(2,:),'--', E,MaxSpringForceE(3,:),':',E,MaxSpringForceE(4,:),'-.','LineWidth',2);
     title({['Maximum compression'] ['as a function of Youngs modulus'] ['t =' num2str(t) 'mm and c =' num2str(c) ', k_0 =' num2str(k) 'kPa/mm']});
    xlabel('E [MPa]');
    ylabel('Compression [mm]');
    grid on
     lgd = legend(string(Load/1000),'location','northeast');
...
semilogy(k,StressdiffForcek(1,:),'-',k,StressdiffForcek(2,:),'--',k,StressdiffForcek(3,:), ':',k,StressdiffForcek(4,:), '-.','LineWidth',2);
     title({['Standard deviation'] ['of the stress distribution as a function of k_0'] ['t =' num2str(t) 'mm and c =' num2str(c) ', E =' num2str(E) 'MPa']});
    xlabel('k_0 [kPa/mm]');
    ylabel('\sigma [kPa]');
    grid on
    lgd = legend(string(Load/1000),'location','northeast');
...
plot(k,MaxSpringForcek(1,:),'-', k,MaxSpringForcek(2,:),'--', k,MaxSpringForcek(3,:),':',k,MaxSpringForcek(4,:),'-.','LineWidth',2);
     title({['Maximum compression'] ['as a function of k_0'] ['t =' num2str(t) 'mm and c =' num2str(c) ', E =' num2str(E) 'MPa']});
    xlabel('k_0 [kPa/mm]');
    ylabel('Compression [mm]');
    grid on
    lgd = legend(string(Load/1000),'location','northeast');
